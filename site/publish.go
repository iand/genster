...
func (ps *PublishSet) Gedcom() (*gedcom.Gedcom, error) {
	gmap := struct {
		individual map[string]*gedcom.IndividualRecord
		family     map[string]*gedcom.FamilyRecord
	}{
		individual: make(map[string]*gedcom.IndividualRecord),
		family:     make(map[string]*gedcom.FamilyRecord),
	}

	getFamily := func(id string) *gedcom.FamilyRecord {
		fr, ok := gmap.family[id]
		if !ok {
	...
	includeIndividual := func(p *model.Person) *gedcom.IndividualRecord {
		ir, ok := gmap.individual[p.ID]
		if !ok {
			ir = new(gedcom.IndividualRecord)
			ir.Xref = p.ID
			gmap.individual[p.ID] = ir
	...
	for _, ir := range gmap.individual {
		g.Individual = append(g.Individual, ir)
	}

	for _, fr := range gmap.family {
